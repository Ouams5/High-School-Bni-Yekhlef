...
import { Send, Users, ArrowLeft, Shield, Crown, Code2, X } from 'lucide-react';
import ReactMarkdown from 'react-markdown';
import remarkGfm from 'remark-gfm';

export const ClubChat = () => {
  const { clubId } = useParams<{ clubId: string }>();
  const { user, checkServiceLimit } = useAuth();
  ...
  const limitation = checkServiceLimit('club_chat');
  ...
  return (
    <div className="flex flex-col h-[calc(100dvh-140px)] bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden font-sans" data-theme={club?.theme || 'light'}>
      ...
      <div className="p-4 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
        ...
        {club?.chatRules && club.chatRules.length > 0 && (
          <button
            onClick={() => setShowRulesModal(true)}
            className="flex items-center gap-1.5 px-3 py-1.5 bg-indigo-50 hover:bg-indigo-100 text-indigo-700 font-bold rounded-lg text-xs transition-colors border border-indigo-100"
          >
            📜 Chat Protocols
          </button>
        )}
      ...
      <div className="p-4 bg-white border-t border-slate-200">
        {limitation.limited ? (
          <div className="p-3 bg-rose-50 border border-rose-200 rounded-xl text-rose-700 text-xs font-semibold text-center leading-relaxed">
            🚫 Access Restricted by Developers/Owners: You are suspended from Club Chat services. <br/>
            <span className="font-bold">Reason:</span> {limitation.reason} <br/>
            <span className="font-bold">Active Until:</span> {limitation.until}
          </div>
        ) : (
      ...
      {/* Chat Rules Modal */}
      {showRulesModal && club?.chatRules && (
        <div className="fixed inset-0 z-50 bg-slate-900/60 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-800 rounded-2xl max-w-md w-full border border-slate-200 dark:border-slate-700 shadow-2xl flex flex-col p-6 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between pb-4 border-b border-slate-100 dark:border-slate-700">
              <h3 className="text-base font-bold text-slate-800 dark:text-white flex items-center gap-2">
                📜 {club.name} Chat Rules
              </h3>
              <button 
                onClick={() => setShowRulesModal(false)}
                className="text-slate-400 hover:text-slate-650 p-1 rounded-full hover:bg-slate-50 dark:hover:bg-slate-750 transition-colors"
              >
                <X size={18} />
              </button>
            </div>
            ...
            <div className="py-4 space-y-3 overflow-y-auto max-h-[350px]">
              {club.chatRules.map((rule, index) => (
                <div key={index} className="flex gap-3 items-start text-xs font-medium text-slate-700 dark:text-slate-300 bg-slate-50 dark:bg-slate-900 p-3 rounded-xl border border-slate-100 dark:border-slate-800">
                  <span className="font-mono font-bold text-indigo-500 bg-indigo-50 dark:bg-indigo-950/40 w-5 h-5 flex items-center justify-center rounded-full shrink-0">
                    {index + 1}
                  </span>
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-slate-700 flex justify-end">
              <button
                onClick={() => setShowRulesModal(false)}
                className="px-5 py-2 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl transition-all shadow-md shadow-indigo-100"
              >
                I Understand
              </button>
            </div>
