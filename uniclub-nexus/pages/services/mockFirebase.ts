...
import { User, UserRole, Club, Announcement, AppEvent, Project, BugReport, Notification, ChatMessage, Credit, ClubBadge } from '../types';
...
const sanitizeUser = (data: any): User => {
    ...
    return {
        ...data,
        role: role,
        clubBadges: data.clubBadges || [] // Ensure array exists
    } as User;
};
...
class FirebaseService {
  ...
  async claimClubBadge(userId: string, badge: ClubBadge): Promise<void> {
      const userRef = doc(firestore, "users", userId);
      await updateDoc(userRef, {
          clubBadges: arrayUnion(badge)
      });
  }
