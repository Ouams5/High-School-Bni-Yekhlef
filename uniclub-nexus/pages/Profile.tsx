...
export const Profile = () => {
  ...
  if (!user) return <div className="p-10 text-center text-slate-500">{t('pleaseSignIn')}</div>;
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
            <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><Award className="text-yellow-500"/> {t('membershipStatus')}</h2>
            <div className="space-y-4">
                <div className="flex justify-between border-b border-slate-50 pb-3">
                    <span className="text-slate-500">{t('clubsJoined')}</span>
                    <span className="font-medium">{user.joinedClubIds?.length || 0}</span>
                </div>
                 <div className="flex justify-between">
                    <span className="text-slate-500">{t('clubLeader')}</span>
        ...
        <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-200">
             <h2 className="text-xl font-bold mb-4 flex items-center gap-2"><BookOpen className="text-blue-500"/> {t('myClubs')}</h2>
             {clubs.length > 0 ? (
