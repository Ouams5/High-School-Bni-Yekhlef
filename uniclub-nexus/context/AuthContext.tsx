...
interface AuthContextType {
  ...
  checkServiceLimit: (serviceName: string) => { limited: boolean; reason?: string; until?: string };
}
...
export const AuthProvider = ({ children }: React.PropsWithChildren<{}>) => {
  ...
  const checkServiceLimit = (serviceName: string): { limited: boolean; reason?: string; until?: string } => {
    if (!user || !user.limitedServices) return { limited: false };
    const { services, until, reason } = user.limitedServices;
    if (!services.includes(serviceName)) return { limited: false };
    ...
    return {
      limited: true,
      reason: reason || "Administrative usage restriction is active.",
      until: until === 'indefinite' ? 'Indefinite' : new Date(until).toLocaleString()
    };
  };

  const canManageClub = (clubId: string) => {
    if (!user) return false;
    if (user.role === UserRole.OWNER || user.role === UserRole.DEV || user.role === UserRole.ADMIN) return true;
    if (user.role === UserRole.CLUB_LEADER && user.leadingClubId === clubId) return true;
    if (user.leadingClubId === clubId) return true;
    return false;
  };
