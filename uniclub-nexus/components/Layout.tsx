      ...
      <div className="flex-1 flex flex-col overflow-hidden relative" style={{ background: 'var(--bg-main)' }}>
        ...
        <header className="h-16 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md sticky top-0 z-30 flex items-center justify-between px-6 border-b border-slate-200/60 dark:border-slate-800">
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="relative">
                <button onClick={() => setShowNotifPanel(!showNotifPanel)} className="relative p-2.5 text-slate-500 hover:text-indigo-600 hover:bg-indigo-50 dark:hover:bg-indigo-900/30 rounded-xl transition-colors" aria-label="Notifications">
                    <Bell size={22} />
                    {notifications.length > 0 && <span className="absolute top-2 right-2 w-2.5 h-2.5 bg-rose-500 rounded-full border-2 border-white dark:border-slate-900 animate-pulse"></span>}
                </button>
            </div>
        ...
        {showNotifPanel && (
            <>
                ...
                />
                <div className={`fixed inset-y-0 ${isRTL ? 'left-0 border-r animate-in slide-in-from-left' : 'right-0 border-l animate-in slide-in-from-right'} w-full sm:w-[460px] md:w-[500px] bg-white dark:bg-slate-900 shadow-2xl z-50 flex flex-col border-slate-200 dark:border-slate-800 duration-300`}>
                    <div className={`p-6 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50/50 dark:bg-slate-900/50 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                        <div className={isRTL ? 'text-right' : 'text-left'}>
                            <h3 className={`text-lg font-black font-display text-slate-800 dark:text-white flex items-center gap-2 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                                <Bell size={20} className="text-indigo-500 shrink-0"/> {isRTL ? 'إشعاراتك' : 'Notifications'}
                            </h3>
                            <p className="text-xs font-semibold text-slate-400 dark:text-slate-500 mt-1 uppercase tracking-wider">
                                {isRTL ? `لديك ${notifications.length} إشعار` : `You have ${notifications.length} new messages`}
                            </p>
                        </div>
                        <button 
                            onClick={() => setShowNotifPanel(false)}
                    ...
                    <div className="flex-1 overflow-y-auto bg-slate-50/30 dark:bg-slate-900/50 p-5 space-y-4 custom-scrollbar">
                        {notifications.length === 0 ? (
                            <div className="h-full flex flex-col items-center justify-center text-slate-400 dark:text-slate-600 space-y-4">
                                <div className="w-16 h-16 bg-slate-105-lightened rounded-2xl flex items-center justify-center relative border border-slate-200/50 dark:border-slate-800/80 bg-slate-105 dark:bg-slate-800">
                                ...
                                <p className="font-bold text-sm text-slate-500 dark:text-slate-400 tracking-tight">{isRTL ? 'لا توجد إشعارات جديدة' : "You're all caught up!"}</p>
                            </div>
                        ) : (
                            notifications.map(n => (
                                <div key={n.id} className="bg-white dark:bg-slate-800 rounded-2xl p-5 border border-slate-150 dark:border-slate-705 shadow-sm relative group hover:shadow-md hover:border-slate-250 dark:hover:border-slate-700 transition-all duration-200 flex flex-col gap-3">
                                    <div className={`flex justify-between items-start gap-4 ${isRTL ? 'flex-row-reverse' : 'flex-row'}`}>
                                        <div className={`flex items-start gap-2.5 min-w-0 ${isRTL ? 'flex-row-reverse text-right' : 'flex-row text-left'}`}>
                                            {n.type === 'alert' && <ShieldAlert size={18} className="text-amber-500 shrink-0 mt-0.5" />}
                                            <div className="min-w-0 flex-1">
                                                <h4 className="font-bold text-sm text-slate-800 dark:text-slate-100 leading-snug break-words">
                                                    {n.title}
                                                </h4>
                                            </div>
                                        </div>
                                        ...
                                        <button 
                                            ...
                                            className="text-slate-300 hover:text-rose-500 hover:bg-rose-50 dark:hover:bg-rose-950/30 rounded-xl p-1.5 transition-all shrink-0"
                                    ...
                                    <p className={`text-xs sm:text-sm text-slate-500 dark:text-slate-400 leading-relaxed font-semibold break-words ${isRTL ? 'text-right' : 'text-left'}`}>
                                        {n.message}
                                    </p>
                                    
                                    <div className={`flex ${isRTL ? 'justify-end' : 'justify-start'}`}>
                                        <span className="text-[10px] font-bold text-indigo-500 dark:text-indigo-400 uppercase tracking-wider bg-indigo-50 dark:bg-indigo-900/30 border border-indigo-100/50 dark:border-indigo-850/50 px-2.5 py-1 rounded-lg">
                                            {new Date(n.date).toLocaleString(language === 'ar' ? 'ar-EG' : (language === 'fr' ? 'fr-FR' : 'en-US'), { dateStyle: 'short', timeStyle: 'short' })}
                                        </span>
                                    </div>
                                </div>
                            ))
                    ...
                    {notifications.length > 0 && (
                        <div className="p-4 border-t border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-900">
                            <button 
                                onClick={() => {
                                    notifications.forEach(n => handleDismissNotification(n.id, { stopPropagation: () => {} } as any));
                                }}
                                className="w-full py-3 text-sm font-bold text-slate-600 hover:text-slate-800 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:hover:bg-slate-700 rounded-xl transition-all"
                            >
                                {isRTL ? 'مسح كل الإشعارات' : 'Clear all notifications'}
                            </button>
                        </div>
                    )}
                </div>
